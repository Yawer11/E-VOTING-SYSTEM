...
const AdminDashboard = () => {
  ...
  const { heroImage, isVotingStarted, setIsVotingStarted } =
  ...
  const getCandidatesData = () => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/candidate`)
      .then((res) => {
  ...
  const getVoterData = () => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/voter`)
      .then((res) => {
  ...
  const handleAdd = () => {
    if (!name || !image || !position) {
      ...
      axios
        .post(`${process.env.REACT_APP_BASE_URL}/candidate/create`, payload, {
          headers: {
  ...
  return (
    <Box pt={"50px"} px={"60px"} bg={"gray.50"} mb={"150px"}>
      ...
      >
        ...
        />
        <Box>
          <Heading color={"teal"} fontSize={"30px"} fontWeight={"500"}>
            Admin Dashboard
          </Heading>
        </Box>
      ...
      <Box
        display={"grid"}
        gridTemplateColumns={{ base: "repeat(1,1fr)", sm: "49% 49%" }}
        gap={"2%"}
      >
        <Box
          border={"1px solid #E0E0E0"}
            ...
            <Box>
              <Box>
                <Heading textAlign="center" mt={"50px"}>
                  No Status Voting not started yet
                </Heading>
              </Box>
            </Box>
          ) : (
            ...
            >
              <Box>
                <Heading color="teal" textAlign="center">
                  {voterFalseData.length}/{voterList.length}
                </Heading>
              </Box>
              <Table variant="striped" colorScheme="teal">
                {voterFalseData.length == 0 ? (
                  <TableCaption>No Voter Left</TableCaption>
                ) : (
        ...
        >
          <Flex justify={"center"}>
            <Stack width={"60%"}>
              ...
              />
              <Button my={"15px"} onClick={() => handleAdd()} bg={"teal"}>
                Add
              </Button>
            </Stack>
          </Flex>
      ...
      <Box>
        {candidateData.length == 0 ? (
          <Heading textAlign="center" mt={"50px"}>
            There is no candidate please add first
          </Heading>
        ) : (
          <TableContainer
            border={"1px solid #E0E0E0"}
          ...
          >
            <Table variant="striped" colorScheme="teal">
              {candidateData.legnth == 0 ? (
                <TableCaption>Add Candidates</TableCaption>
              ) : (
                <TableCaption></TableCaption>
              )}
              <Thead>
